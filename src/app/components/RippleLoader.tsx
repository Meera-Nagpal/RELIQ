  ...
  return (
    ...
    >
      <style>{`
        ...
        .reliq-ripple-ring-1 {
          animation: reliqRippleAnimation ${mode === 'pulse' ? '0.7s' : '1.8s'} cubic-bezier(0.22, 0.61, 0.36, 1) ${mode === 'pulse' ? 'forwards' : 'infinite'};
          animation-delay: 0s;
        }

        .reliq-ripple-ring-2 {
          animation: reliqRippleAnimation ${mode === 'pulse' ? '0.7s' : '1.8s'} cubic-bezier(0.22, 0.61, 0.36, 1) ${mode === 'pulse' ? 'forwards' : 'infinite'};
          animation-delay: ${mode === 'pulse' ? '0.15s' : '0.45s'};
        }

        .reliq-ripple-ring-3 {
          animation: reliqRippleAnimation ${mode === 'pulse' ? '0.7s' : '1.8s'} cubic-bezier(0.22, 0.61, 0.36, 1) ${mode === 'pulse' ? 'forwards' : 'infinite'};
          animation-delay: ${mode === 'pulse' ? '0.3s' : '0.9s'};
        }
