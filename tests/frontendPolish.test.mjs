...
export async function runFrontendPolishTests() {
  ...
  // -------------------------------------------------------------
  // Test 13: 3D Spatial typography background positioning and dark muted DETECT
  // -------------------------------------------------------------
  test('13. 3D Spatial typography DETECT is restored to dark muted tone (#3A180E) with background z-space', () => {
    const stPath = path.join(ROOT, 'src/experience/SpatialTypography.tsx');
    const stContent = fs.readFileSync(stPath, 'utf8');

    // State 3 COMPARE at y = -0.2
    assert.ok(stContent.includes("position={[2.2, -0.2, -1.8]}"), 'State 3 COMPARE must be positioned at y = -0.2');
    // State 4 DETECT at z = -2.4 with dark tone #3A180E and fillOpacity 0.20
    assert.ok(stContent.includes("position={[0, 1.8, -2.4]}"), 'State 4 DETECT must be placed at z = -2.4');
    assert.ok(stContent.includes('color="#3A180E"'), 'State 4 DETECT must be restored to dark muted tone #3A180E');
    assert.ok(stContent.includes('fillOpacity={0.20}'), 'State 4 DETECT must have atmospheric low opacity 0.20');
